...
Token* readString(void){
  ...
  while ((currentChar != EOF) && (state <1)){
    ...
    token->stringValue[i] = currentChar;
    i++;
    readChar();
  }
  token->stringValue[i-1]='\0';
  if (state != 1){
...
Token* getToken(void) {
  ...
  case CHAR_PERIOD:
    ...
    if ((currentChar != EOF) && (charCodes[currentChar] == CHAR_RPAR)) {
      ...
      Token *t = makeToken(TK_DOUBLE, lineNo, colNo);
		  int count = 2;
		  int countDot = 0;
		  t->string[0] = '0';
		  t->string[1] = '.';
		  while(charCodes[currentChar] == CHAR_DIGIT || charCodes[currentChar] == CHAR_PERIOD) {
      		...
      		countDot++;
  			if(countDot>0)
    			error(ERR_INVALID_CONSTANT_CHAR, t->lineNo, t->colNo);
  			  t->string[count++] = currentChar;
  			readChar();
		    }	
		  t->string[count] = '\0';
		  return t;
