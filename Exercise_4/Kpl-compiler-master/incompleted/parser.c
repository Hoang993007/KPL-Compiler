...
ConstantValue* compileUnsignedConstant(void) {
  ...
  case TK_DOUBLE:
    eat(TK_DOUBLE);
    constValue = makeDoubleConstant(currentToken->string);
    break;
  case TK_STRING:
    eat(TK_STRING);
...
//hoangdz
//tao 2 mang luu cac bien o 2 ve => so sanh cac phan tu trong 2 mang
void compileAssignSt(void) {
  ...
  Type *LType = NULL, *RType;
  Type *LTypeArr[100], *RTypeArr[100];
  int countL = 0, countR = 0;
  LType = compileLValue();
  LTypeArr[countL] = LType;
  countL++;
  ...
  for(int i = 0; i < countL; i++){
    checkTypeEquality(LTypeArr[i], RTypeArr[i]);
  }
}
